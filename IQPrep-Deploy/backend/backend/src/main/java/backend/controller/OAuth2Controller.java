...
public class OAuth2Controller {
    ...
    private static boolean isOAuthClientMissing(String clientId) {
        return clientId == null || clientId.isBlank() || "NOT_SET".equals(clientId);
    }
    ...
    public ResponseEntity<?> googleUrl() {
        if (isOAuthClientMissing(googleClientId))
            return ResponseEntity.status(503).body(Map.of(
                "message", "Google OAuth not configured. Set google.client.id (or GOOGLE_CLIENT_ID)."));
    ...
    public ResponseEntity<?> githubUrl() {
        if (isOAuthClientMissing(githubClientId))
            return ResponseEntity.status(503).body(Map.of(
                "message", "GitHub OAuth not configured. Set github.client.id (or GITHUB_CLIENT_ID)."));
